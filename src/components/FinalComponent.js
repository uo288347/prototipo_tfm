...
export const FinalComponent = ({ }) => {
    ...
    const shareUrl = 'https://carbayo.vercel.app/';
    const shareText = t('end.shareMessage') || "¡Te invitamos a participar en un estudio académico de la Universidad de Oviedo!";

    // ── Helper: convierte el card en un File de imagen ──
    const buildImageFile = async () => {
        if (!cardRef.current) return null;
        const dataUrl = await toPng(cardRef.current, { pixelRatio: 2, cacheBust: true });
        const blob = await (await fetch(dataUrl)).blob();
        return new File([blob], 'invitacion-estudio.png', { type: 'image/png' });
    };
    ...
    const handleWhatsApp = async () => {
        ...
        try {
            ...
            // Móvil con API nativa (Android / iOS)
            if (file && navigator.canShare?.({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: t('end.shareTitle') || "Estudio Universidad de Oviedo",
                    text: shareUrl,
                });
                return;
            }

            // Escritorio: descargar imagen + abrir WhatsApp Web con texto
            if (file) {
                const a = document.createElement("a");
                a.href = URL.createObjectURL(file);
                a.download = "invitacion-estudio.png";
                a.click();
            }
            window.open(
            ...
            // Fallback si el usuario cancela o algo falla
            window.open(
    ...
    // ── Compartir nativo (cualquier app) ─────────────────────────────────────
    const handleNativeShare = async () => {
        ...
        try {
            ...
            if (file && navigator.canShare?.({ files: [file] })) {
                ...
                await navigator.share({
                    title: t('end.shareTitle') || "Estudio",
                    text: shareText,
                    url: shareUrl,
                });
            }
...
// ── Twitter ───────────────────────────────────────────────────────────────
    ...
    return (
        <div style={{
            ...
            {/* ── Tarjeta oculta que se convierte en imagen ── */}
            <div style={{ position: "absolute", left: "-9999px", top: "-9999px" }}>
                <InvitationCard ref={cardRef} shareText={shareText} t={t} />
            </div>
            ...
            <div style={{ marginTop: "24px", textAlign: "center" }}>
                ...
                <div style={{
                    ...
                    {navigator.share && (
                        <ShareButton
                            label={generatingImage ? "Generando..." : (t('end.shareMore') || "Más opciones")}
                            color="#61a4fc"
                            hoverColor="#1877F2"
                    ...
                    <ShareButton
                        label={generatingImage ? "Generando..." : "WhatsApp"}
                        color="#25D366"
                        hoverColor="#1ebe57"
