...
export const GhostProductCard = ({ dragGhostPosition, selectedItems, products, locale }) => {
    ...
    return (
        ...
        >
            <div style={{ position: 'relative' }}>
                ...
                {Array.from(selectedItems).slice(0, 2).map((itemKey, index) => {
                    const [productId, size] = itemKey.split('-');
                    const item = products.find(p => p.id === productId && p.size === size);
                    if (!item) return null;

                    const product = getProduct(item.id);
                    const productTitle = getProductTitle(item.id, locale || 'es');
                    ...
                    return (
                        ...
                        {/* Badge con el número total si hay más de 2 items */}
